...
public:
  ...
  float get_start_rt() override { return data_file_.index.empty() ? (chromatograms_.empty() ? 0.0f : chromatograms_.front().time_minutes.front()) : data_file_.retention_time_start_ms / 60000.0f; }
  float get_end_rt() override { return data_file_.index.empty() ? (chromatograms_.empty() ? 0.0f : chromatograms_.front().time_minutes.back()) : data_file_.retention_time_end_ms / 60000.0f; }
  ...
  std::vector<float> get_spectra_rt(std::vector<int> indices = {}) override { return selected_float(std::move(indices), [](const auto &entry) { return entry.retention_time_ms / 60000.0f; }); }
  ...
  MASS_SPEC_SPECTRA_HEADERS get_spectra_headers(std::vector<int> indices = {}, bool = false) override
  {
    ...
    for (std::size_t n = 0; n < selected.size(); ++n) { const auto spectrum = read_spectrum(data_file_, selected[n]); out.index[n] = static_cast<int>(selected[n]); out.scan[n] = static_cast<int>(selected[n] + 1); out.array_length[n] = static_cast<int>(spectrum.mz.size()); out.level[n] = 1; out.rt[n] = spectrum.retention_time_ms / 60000.0f; out.lowmz[n] = spectrum.mz.empty() ? 0.0f : spectrum.mz.front(); out.highmz[n] = spectrum.mz.empty() ? 0.0f : spectrum.mz.back(); out.bpint[n] = base_peak(data_file_, selected[n], false); out.bpmz[n] = base_peak(data_file_, selected[n], true); out.tic[n] = std::accumulate(spectrum.intensity.begin(), spectrum.intensity.end(), 0.0f); }
    return out;
  }
  ...
  std::vector<std::vector<std::vector<float>>> get_chromatograms(std::vector<int> indices = {}) override { std::vector<std::vector<std::vector<float>>> out; for (const auto index : detail::normalize(std::move(indices), chromatograms_.size())) out.push_back({chromatograms_[index].time_minutes, chromatograms_[index].intensity}); return out; }
  ...
  {
    const auto spectrum = read_spectrum(data_file_, static_cast<std::size_t>(index)); MASS_SPEC_SPECTRUM out{}; out.index = index; out.scan = index + 1; out.array_length = static_cast<int>(spectrum.mz.size()); out.level = 1; out.rt = spectrum.retention_time_ms / 60000.0f; out.binary_arrays_count = 2; out.binary_names = {"m/z", "intensity"}; out.binary_data = {spectrum.mz, spectrum.intensity}; out.lowmz = spectrum.mz.empty() ? 0.0f : spectrum.mz.front(); out.highmz = spectrum.mz.empty() ? 0.0f : spectrum.mz.back(); out.tic = std::accumulate(spectrum.intensity.begin(), spectrum.intensity.end(), 0.0f); auto it = std::max_element(spectrum.intensity.begin(), spectrum.intensity.end()); if (it != spectrum.intensity.end()) { out.bpint = *it; out.bpmz = spectrum.mz[static_cast<std::size_t>(std::distance(spectrum.intensity.begin(), it))]; } return out;
  }
private:
