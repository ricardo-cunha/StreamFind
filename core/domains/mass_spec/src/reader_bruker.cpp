...
public:
  ...
  {
    const auto &frame = frames_.at(static_cast<std::size_t>(index)); const auto raw = read_tsf_line_spectrum(file_, frame); const auto calibration = read_tsf_calibration(file_, frame); const auto mz = tsf_tof_to_mz(calibration, raw.tof); MASS_SPEC_SPECTRUM out{}; out.index = index; out.scan = static_cast<int>(frame.id); out.array_length = static_cast<int>(mz.size()); out.level = frame.msms_type == 0 ? 1 : 2; out.mode = frame.scan_mode; out.polarity = frame.polarity == "+" ? 1 : frame.polarity == "-" ? -1 : 0; out.lowmz = 95.0f; out.highmz = 2505.0f; out.bpint = static_cast<float>(frame.max_intensity); out.tic = static_cast<float>(frame.summed_intensities); out.rt = static_cast<float>(frame.retention_time); out.binary_arrays_count = 2; out.binary_names = {"m/z", "intensity"}; out.binary_data.resize(2); out.binary_data[0].reserve(mz.size()); out.binary_data[1].reserve(raw.intensity.size()); for (std::size_t n = 0; n < mz.size(); ++n) { out.binary_data[0].push_back(static_cast<float>(mz[n])); out.binary_data[1].push_back(static_cast<float>(raw.intensity[n])); } return out;
  }
private:
...
public:
  ...
  MASS_SPEC_SPECTRUM get_spectrum(const int &index) override
  {
    ...
    for (std::size_t n = 0; n < profile.intensity.size(); ++n) { out.binary_data[0][n] = static_cast<float>(s.mz_lower + step * static_cast<double>(n)); out.binary_data[1][n] = static_cast<float>(profile.intensity[n]); }
    return out;
  }
private:
