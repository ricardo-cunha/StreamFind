...
namespace mass_spec
{
  ...
  namespace reader
  {
    ...
    namespace shimadzu_lcd
    {
      ...
      const TlmMethodTransitionSet *find_method_transition_set(const std::vector<TlmMethodTransitionSet> &sets, const MS_SPECTRUM &spectrum)
      {
      ...
      std::vector<MS_SPECTRUM> parse_tlm_spectra(const std::string &file_path, const std::vector<TlmMethodTransitionSet> &method_sets)
      {
        const auto streams = ole::list_streams(file_path);
      ...
      int find_transition_key(const std::vector<TlmTransitionKey> &keys, const MS_SPECTRUM &spectrum)
      {
      ...
      std::vector<text_chromatogram::Block> build_tlm_chromatograms(const std::vector<MS_SPECTRUM> &spectra, const std::vector<TlmMethodTransitionSet> &method_sets)
      {
        std::vector<TlmTransitionKey> keys;
        std::vector<std::vector<const MS_SPECTRUM *>> grouped;
      ...
      MS_SPECTRA_HEADERS Reader::get_spectra_headers(std::vector<int> indices, bool)
      {
        indices = normalized_indices(std::move(indices), pimpl->spectra.size());
        MS_SPECTRA_HEADERS h;
        h.resize_all(static_cast<int>(indices.size()));
