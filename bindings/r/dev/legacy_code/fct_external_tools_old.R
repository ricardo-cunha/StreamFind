...
# Boost, RDKit, LPSolve, MetFrag, CFM-ID, vcpkg) in ~/.streamfind/tools/
...
.sf_source_root <- function() {
  ...
  for (candidate in candidates) {
    if (file.exists(file.path(candidate, "DESCRIPTION")) && dir.exists(file.path(candidate, "src"))) {
      return(candidate)
    }
  }
...
.sf_find_existing_path <- function(paths, must_be_dir = FALSE) {
  for (path in paths) {
    if (!nzchar(path)) {
      next
    }
    ...
    if (!must_be_dir && file.exists(path)) {
      return(normalizePath(path, winslash = "/", mustWork = FALSE))
    }
  }
...
.sf_duckdb_library_filename <- function() {
  if (.Platform$OS.type == "windows") {
    return("duckdb.dll")
  }
...
.sf_resource_path <- function(...) {
  source_root <- .sf_source_root()
  relative <- file.path(...)
  candidates <- c(
    system.file(..., package = "streamfind"),
    file.path(source_root, "inst", relative),
    file.path(source_root, relative)
  )

  .sf_find_existing_path(candidates, must_be_dir = TRUE)
}

.sf_user_tool_path <- function(tool, version = NULL) {
  parts <- c(get_external_dir(), tool, version)
  do.call(file.path, as.list(parts[!is.na(parts) & nzchar(parts)]))
}

.sf_openbabel_data_path <- function() {
  source_root <- .sf_source_root()
  candidates <- c(
    Sys.getenv("STREAMFIND_OPENBABEL_DATA", unset = ""),
    system.file("extdata", "openbabel", "data", package = "streamfind"),
    file.path(source_root, "inst", "extdata", "openbabel", "data"),
    file.path(source_root, "src", "core", "external", "openbabel", "openbabel-3-2-0", "data"),
    file.path(.sf_user_tool_path("openbabel"), "data")
  )

  .sf_find_existing_path(candidates, must_be_dir = TRUE)
}

.sf_duckdb_library_path <- function() {
  platform_tag <- .sf_platform_tag()
  library_name <- .sf_duckdb_library_filename()
  source_root <- .sf_source_root()

  package_candidates <- if (.Platform$OS.type == "windows") {
    c(
      system.file("libs", "x64", library_name, package = "streamfind"),
      system.file("libs", library_name, package = "streamfind")
    )
  } else {
    c(system.file("libs", library_name, package = "streamfind"))
  }

  candidates <- c(
    Sys.getenv("STREAMFIND_DUCKDB_LIBRARY", unset = ""),
    package_candidates,
    file.path(source_root, "inst", "libs", "x64", library_name),
    file.path(source_root, "inst", "libs", library_name),
    file.path(source_root, "src", "core", "external", "duckdb", "lib", platform_tag, library_name),
    file.path(.sf_user_tool_path("duckdb"), library_name)
  )

  .sf_find_existing_path(candidates, must_be_dir = FALSE)
}

.sf_configure_native_runtime <- function() {
  openbabel_data <- .sf_openbabel_data_path()
  if (!is.na(openbabel_data)) {
    Sys.setenv(
      STREAMFIND_OPENBABEL_DATA = openbabel_data,
      BABEL_DATADIR = openbabel_data
    )
  }

  duckdb_library <- .sf_duckdb_library_path()
  if (!is.na(duckdb_library)) {
    Sys.setenv(STREAMFIND_DUCKDB_LIBRARY = duckdb_library)

    if (.Platform$OS.type == "windows") {
      .path_prepend(dirname(duckdb_library))
    } else if (grepl("\\.(so|dylib)$", duckdb_library, ignore.case = TRUE)) {
      .libpath_prepend(dirname(duckdb_library))
    }
  }
...
#'
#' Returns (and creates if needed) \code{~/.streamfind/tools/}.
#' @return The normalized path to \code{~/.streamfind/tools/}.
#' @export
...
#'
#' Checks PATH first, then scans \code{~/.streamfind/tools/java/}.
#' @return Path to \code{java} (or \code{java.exe}), or \code{NA} if not found.
#' @export
get_java_path <- function() {
  java_path <- Sys.which("java")
  if (nzchar(java_path)) return(normalizePath(java_path, winslash = if (.Platform$OS.type == "windows") "\\" else "/"))
...
#'
#' Checks \code{~/.streamfind/tools/lpsolve/5.5.2.11/}.
...
#'
#' Checks \code{~/.streamfind/tools/conda/}.
...
#'
#' Checks \code{~/.streamfind/tools/vcpkg/}.
...
#'
#' Checks \code{~/.streamfind/tools/metfrag/MetFragCL.jar}.
...
#'
#' Checks \code{~/.streamfind/tools/cfm-id/bin/} for native binaries
...
#' Check status of optional external tools
#'
#' Scans \code{~/.streamfind/tools/} and the system PATH to report which
...
#' Report bundled and user-managed external paths
#'
#' @return A named list describing the configured bundled-resource and
#'   user-managed external-tool paths, including the Java executable and the
#'   deterministic MetFrag CL jar when available.
#' @export
...
#' Report bundled OpenBabel runtime information
#'
#' @return A named list with bundled OpenBabel data-path and runtime diagnostics.
#' @export
get_openbabel_info <- function() {
  debug_runtime <- tryCatch(
    rcpp_openbabel_debug_runtime(),
    error = function(e) conditionMessage(e)
  )
  example_svg <- tryCatch(
    rcpp_openbabel_structure_svg(SMILES = "CCO", width = 80L, height = 80L),
    warning = function(w) "",
    error = function(e) ""
  )

  list(
    compiled_support = isTRUE(nzchar(example_svg)),
    data_path = .sf_openbabel_data_path(),
    env_data_path = Sys.getenv("STREAMFIND_OPENBABEL_DATA", unset = NA_character_),
    babel_datadir = Sys.getenv("BABEL_DATADIR", unset = NA_character_),
    runtime_debug = debug_runtime
  )
}

#' Report bundled DuckDB runtime information
#'
#' @return A named list with bundled DuckDB library details.
#' @export
get_duckdb_info <- function() {
  ...
  list(
    bundled_library = .sf_duckdb_library_path(),
    env_library = Sys.getenv("STREAMFIND_DUCKDB_LIBRARY", unset = NA_character_),
    duckdb_r_package_version = duckdb_version
  )
}

#' Check bundled tool availability
#'
#' @param verbose If `TRUE`, print a short status summary.
#' @return A named list with bundled OpenBabel, DuckDB, and external-path
#'   information.
#' @export
check_bundled_tools <- function(verbose = TRUE) {
  bundled <- list(
    openbabel = get_openbabel_info(),
    duckdb = get_duckdb_info(),
    paths = get_external_paths()
  )

  if (verbose) {
    message("[streamfind] Java: ", bundled$paths$java)
    message("[streamfind] MetFrag CL: ", bundled$paths$metfrag)
    message("[streamfind] Bundled OpenBabel data: ", bundled$openbabel$data_path)
    message("[streamfind] Bundled DuckDB library: ", bundled$duckdb$bundled_library)
    message("[streamfind] User tools directory: ", bundled$paths$tools)
  }
...
install_cfm_id_docker <- function(quiet = TRUE) {
...
install_rdkit_from_conda <- function(quiet = TRUE) {
  conda_dir <- .ext_conda_dir()
  conda_exe <- if (.Platform$OS.type == "windows") {
    file.path(conda_dir, "Scripts", "conda.exe")
  } else {
...
#' The active flow currently focuses on Java and MetFrag; the other installers
#' remain in the file but are commented out in this dispatcher for now.
#'
#' @param tools Character vector of tool names to install. Default installs
#'   the Java runtime and MetFrag CLI bundle.
...
install_external_tools <- function(
  ...
  #
  # The installers below are intentionally left in the file for later use,
  # but commented out while the CLI tool flow is narrowed to Java + MetFrag.
  ...
  # if ("rdkit_conda" %in% tools)   ok <- ok && isTRUE(install_rdkit_from_conda(quiet = quiet))
  ...
  # if ("cfm_id_docker" %in% tools) ok <- ok && isTRUE(install_cfm_id_docker(quiet = quiet))
  invisible(ok)
}
