...
namespace nts
{
  ...
  PROJECT_NON_TARGET_ANALYSIS::PROJECT_NON_TARGET_ANALYSIS(std::shared_ptr<project::CONTEXT> ctx)
      : ctx_(std::move(ctx))
  {
  ...
  void PROJECT_NON_TARGET_ANALYSIS::create_schema(const std::shared_ptr<project::CONTEXT> &ctx)
  {
    auto guard = connect_checked(ctx);
    detail::run_sql(
        guard.get(),
  ...
  void PROJECT_NON_TARGET_ANALYSIS::validate_schema(const std::shared_ptr<project::CONTEXT> &ctx)
  {
    auto guard = connect_checked(ctx);
