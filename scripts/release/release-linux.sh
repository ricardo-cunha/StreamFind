...
#
# Invoked by scripts/release/release.ps1 (-Linux) via WSL.
#
# Design points:
...
# The Linux release runs only the official C++ and Rust suites.
...
if [ "${STREAMFIND_BUILD_CORE:-1}" != "0" ]; then
    ...
    if [ "${STREAMFIND_RUN_TESTS:-1}" != "0" ]; then
        echo "[release-linux] running official C++ tests..."
        (cd "$CORE_BUILD" && ctest -C Release --output-on-failure)
    fi
...
if [ "${STREAMFIND_BUILD_RUST:-1}" != "0" ]; then
    ...
    if [ "${STREAMFIND_RUN_TESTS:-1}" != "0" ]; then
        echo "[release-linux] running official Rust tests..."
