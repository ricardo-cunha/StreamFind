...
#' @param owner_class A character string representing the owning project class for this step
#'   (e.g., `ProjectMassSpec` or `ProjectNonTargetAnalysis`). If `NA`, a default owner class
#'   is inferred from the input/output classes when possible.
...
ProcessingStep <- function(
  ...
  x <- structure(
    list(
      ...
      required = required,
      algorithm = algorithm,
      owner_class = if (is.na(owner_class)) {
        .infer_processing_owner_class(type, input_class, output_class)
      } else {
        owner_class
      },
...
validate_object.ProcessingStep <- function(x) {
  ...
  checkmate::assert_character(x$owner_class, len = 1)
...
as.ProcessingStep <- function(value) {
  ...
  if (is.list(value)) {
    ...
    if (!"owner_class" %in% names(value)) {
      value$owner_class <- .infer_processing_owner_class(
        value$type,
