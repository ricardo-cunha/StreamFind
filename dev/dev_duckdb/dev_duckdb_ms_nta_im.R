project_db <- file.path("dev", "dev_duckdb", "data_nta_im.duckdb")
project_id <- "demo_nta_im"
...
htmlwidgets::saveWidget(
  plot_raw_spectra_eic_heatmap(
    nta,
    mass = suspects$mass[2:3],
    ppm = 20,
    reduction = 0.15,
    groupBy = c("analysis", "id")
  ),
  file = file.path("dev", "dev_duckdb", "eic_heatmap_plot.html"),
  selfcontained = TRUE
)
...
htmlwidgets::saveWidget(
  ...
  file = file.path("dev", "dev_duckdb", "tic_3d_plot.html"),
  selfcontained = TRUE
)

htmlwidgets::saveWidget(
  plot_raw_spectra_eic_3d(
    nta,
    mass = suspects$mass[2:6],
    ppm = 20,
    useMobility = TRUE,
    reduction = 0.2,
    groupBy = c("id")
  ),
  file = file.path("dev", "dev_duckdb", "eic_3d_plot.html"),
  selfcontained = TRUE
)
