...
///
/// `STREAMFIND_SHIMADZU_DATA_ROOT` is the portable override for developer
/// machines. The E: location is the default on Windows; WSL sees that drive at
/// `/mnt/e`.
pub fn shimadzu_data_dir() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("STREAMFIND_SHIMADZU_DATA_ROOT") {
        let path = PathBuf::from(path);
        return path.is_dir().then_some(path);
    }
    [
        PathBuf::from(r"E:\example_files\raw_vendor_files\shimadzu"),
        PathBuf::from("/mnt/e/example_files/raw_vendor_files/shimadzu"),
    ]
    .into_iter()
